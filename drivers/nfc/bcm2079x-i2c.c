...
static void bcm2079x_set_platform_data(struct i2c_client *client, struct bcm2079x_platform_data *platform_data)
{
	int r;
	u32 tmp;
	
    r = of_property_read_u32( client->dev.of_node, "bcm2079x-i2c,irq_gpio", &tmp);
	if(r == 0){
		platform_data->irq_gpio=tmp;
	}else{
		dev_err(&client->dev, "need bcm2079x-i2c,irq_gpio in dtsi file\n");
	}
	
    r = of_property_read_u32( client->dev.of_node, "bcm2079x-i2c,en_gpio", &tmp);
	if(r == 0){
		platform_data->en_gpio=tmp;
	}else{
		dev_err(&client->dev, "need bcm2079x-i2c,en_gpio in dtsi file\n");
	}

	r = of_property_read_u32( client->dev.of_node, "bcm2079x-i2c,wake_gpio", &tmp);
	if(r == 0){
		platform_data->wake_gpio=tmp;
	}else{
		dev_err(&client->dev, "need bcm2079x-i2c,wake_gpio in dtsi file\n");
	}
	return;
}

static struct of_device_id bcm2079X_i2c_table [] = {
    { . compatible = "sharp,bcm2079x-i2c" ,},
    { },
};
#endif
...
#ifdef CONFIG_OF
	struct bcm2079x_platform_data platform_data_tmp;
	bcm2079x_set_platform_data(client, &platform_data_tmp);
	platform_data = &platform_data_tmp;
#else
...
static struct i2c_driver bcm2079x_driver = {
	...
	.driver = {
		...
		.name = "bcm2079x-i2c",
#ifdef CONFIG_OF
		.of_match_table = bcm2079X_i2c_table,
#endif
	},
};
