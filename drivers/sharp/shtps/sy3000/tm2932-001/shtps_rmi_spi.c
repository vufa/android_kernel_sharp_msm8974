...
#define SHTPS_PEN_HAND_FAIL_EVENT_REPORT_REJECTION
 ...
 */
	...
	#define SHTPS_DISABLE_MULTIPLE_KEY
...
//	#define SHTPS_LPWG_GRIP_SUPPORT_ENABLE
	...
	#if defined(SHTPS_PROXIMITY_SUPPORT_ENABLE)
	//	#define SHTPS_LPWG_PROXIMITY_ASYNC_CHECK_ENABLE
	#endif /* SHTPS_PROXIMITY_SUPPORT_ENABLE */
...
#define SHTPS_TOUCH_CANCEL_COORDINATES_Y		9999
	...
	#define SHTPS_LPWG_REPORT_WG_ONLY						0x02
		...
		static int SHTPS_LPWG_DOZE_WAKEUP_THRESHOLD_DEF = 	0x78;
		static int SHTPS_LPWG_DOZE_WAKEUP_THRESHOLD =	 	0xFF;
		...
		static int SHTPS_LPWG_TX_CLIP_LO = 					0x00;
		static int SHTPS_LPWG_TX_CLIP_HI = 					0x00;
		...
		#define SHTPS_LPWG_DOZE_WAKEUP_THRESHOLD_DEF		0x78
		#define SHTPS_LPWG_DOZE_WAKEUP_THRESHOLD			0xFF
		...
		#define SHTPS_LPWG_TX_CLIP_LO						0x00
		#define SHTPS_LPWG_TX_CLIP_HI						0x00
...
#if defined( SHTPS_LPWG_PROXIMITY_ASYNC_CHECK_ENABLE )
	#if defined( SHTPS_DEBUG_VARIABLE_DEFINES )
	...
	#endif /* SHTPS_DEBUG_VARIABLE_DEFINES ) */
#endif /* SHTPS_LPWG_PROXIMITY_ASYNC_CHECK_ENABLE */
...
#if defined(SHTPS_PEN_HAND_FAIL_EVENT_REPORT_REJECTION)
	#if defined( SHTPS_DEBUG_VARIABLE_DEFINES )
		static int SHTPS_PEN_IGNORE_SURVIVE_AREA =			100;
		static int SHTPS_PEN_IGNORE_SURVIVE_TIME_MS =		100;

		#if defined( SHTPS_MODULE_PARAM_ENABLE )
			module_param(SHTPS_PEN_IGNORE_SURVIVE_AREA, int, S_IRUGO | S_IWUSR);
			module_param(SHTPS_PEN_IGNORE_SURVIVE_TIME_MS, int, S_IRUGO | S_IWUSR);
		#endif /* SHTPS_MODULE_PARAM_ENABLE */
	#else /* #if defined( SHTPS_DEBUG_VARIABLE_DEFINES ) */
		#define SHTPS_PEN_IGNORE_SURVIVE_AREA			100
		#define SHTPS_PEN_IGNORE_SURVIVE_TIME_MS		100
	#endif /* #if defined( SHTPS_DEBUG_VARIABLE_DEFINES ) */
#endif /* SHTPS_PEN_HAND_FAIL_EVENT_REPORT_REJECTION */

#if defined( SHTPS_HOVER_REJECT_ENABLE )
	#if defined( SHTPS_DEBUG_VARIABLE_DEFINES )
		static int SHTPS_HOVER_REJECT_ENABLE_SWITCH =						1;
		static int SHTPS_HOVER_REJECT_FINGER_TU_PEN_IGNORE_TIME =			500;
		static int SHTPS_HOVER_REJECT_FINGER_TU_PEN_IGNORE_MOVE_THRESHOLD =	48;

		static int SHTPS_HOVER_REJECT_CANCEL_MOVE_THRESHOLD =				16;
		static int SHTPS_HOVER_REJECT_FINGER_TU_FINGER_IGNORE_TIME =		35;
		static int SHTPS_HOVER_REJECT_FINGER_TU_FINGER_TD_IGNORE_TIME =		40;
		static int SHTPS_HOVER_REJECT_FINGER_TU_FINGER_IGNORE_TD_COUNT =	3;

		#if defined( SHTPS_MODULE_PARAM_ENABLE )
			module_param(SHTPS_HOVER_REJECT_ENABLE_SWITCH, int, S_IRUGO | S_IWUSR);
			module_param(SHTPS_HOVER_REJECT_FINGER_TU_PEN_IGNORE_TIME, int, S_IRUGO | S_IWUSR);
			module_param(SHTPS_HOVER_REJECT_FINGER_TU_PEN_IGNORE_MOVE_THRESHOLD, int, S_IRUGO | S_IWUSR);

			module_param(SHTPS_HOVER_REJECT_CANCEL_MOVE_THRESHOLD, int, S_IRUGO | S_IWUSR);
			module_param(SHTPS_HOVER_REJECT_FINGER_TU_FINGER_IGNORE_TIME, int, S_IRUGO | S_IWUSR);
			module_param(SHTPS_HOVER_REJECT_FINGER_TU_FINGER_TD_IGNORE_TIME, int, S_IRUGO | S_IWUSR);
			module_param(SHTPS_HOVER_REJECT_FINGER_TU_FINGER_IGNORE_TD_COUNT, int, S_IRUGO | S_IWUSR);
		#endif /* SHTPS_MODULE_PARAM_ENABLE */
	#else /* SHTPS_DEBUG_VARIABLE_DEFINES */
		#define SHTPS_HOVER_REJECT_ENABLE_SWITCH						1
		#define SHTPS_HOVER_REJECT_FINGER_TU_PEN_IGNORE_TIME			500
		#define SHTPS_HOVER_REJECT_FINGER_TU_PEN_IGNORE_MOVE_THRESHOLD	48

		#define SHTPS_HOVER_REJECT_CANCEL_MOVE_THRESHOLD				16
		#define SHTPS_HOVER_REJECT_FINGER_TU_FINGER_IGNORE_TIME			35
		#define SHTPS_HOVER_REJECT_FINGER_TU_FINGER_TD_IGNORE_TIME		40
		#define SHTPS_HOVER_REJECT_FINGER_TU_FINGER_IGNORE_TD_COUNT		3
	#endif /* SHTPS_DEBUG_VARIABLE_DEFINES */
...
struct shtps_rmi_spi {
	...
	#if defined(SHTPS_PEN_HAND_FAIL_EVENT_REPORT_REJECTION)
		u8							pen_hand_ignore_id[SHTPS_FINGER_MAX];
		u8							palm_effect_id[SHTPS_FINGER_MAX];
		u8							palm_effect_survive_id[SHTPS_FINGER_MAX];
		unsigned long				palm_effect_survive_time[SHTPS_FINGER_MAX];
		unsigned short				palm_area_x_min[SHTPS_FINGER_MAX];
		unsigned short				palm_area_x_max[SHTPS_FINGER_MAX];
		unsigned short				palm_area_y_min[SHTPS_FINGER_MAX];
		unsigned short				palm_area_y_max[SHTPS_FINGER_MAX];
	#endif /* SHTPS_PEN_HAND_FAIL_EVENT_REPORT_REJECTION */
...
#if defined(SHTPS_LPWG_PROXIMITY_ASYNC_CHECK_ENABLE) || defined( SHTPS_KEY_PROXIMITY_ASYNC_CHECK_ENABLE )
...
#endif /* SHTPS_LPWG_PROXIMITY_ASYNC_CHECK_ENABLE || SHTPS_KEY_PROXIMITY_ASYNC_CHECK_ENABLE */
...
static int shtps_init_param(struct shtps_rmi_spi *ts)
{
	...
	if( ts->bt_ver == 0x0102 ){
		...
		{
			u8 data;
			rc = shtps_rmi_read(ts, ts->map.fn12.ctrl.num[23].addr, &data, 1);
			if(rc == 0){
				data &= (~0x02);
				rc = shtps_rmi_write(ts, ts->map.fn12.ctrl.num[23].addr, data);
			}
			if(rc != 0){
				SHTPS_LOG_ERR_PRINT("Disable Pen failed!\n");
			}
		}
	...
	if(ts->map.fn1A.enable){
		...
		#if !defined( SHTPS_DISABLE_MULTIPLE_KEY )
			rc = shtps_rmi_write(ts, ts->map.fn1A.ctrlBase, 0x00);
			SPI_ERR_CHECK(rc, err_exit);
		#endif /* #if !defined( SHTPS_DISABLE_MULTIPLE_KEY ) */
...
static void shtps_ignore_touch_area_check(struct shtps_rmi_spi *ts, struct shtps_touch_info *info)
{
	#if defined(SHTPS_PEN_HAND_FAIL_EVENT_REPORT_REJECTION)
	{
		int i, j;
		...
		for(i = 0; i < fingerMax; i++){
			if(ts->palm_effect_survive_id[i] != 0){
				if( time_after(jiffies, ts->palm_effect_survive_time[i]) == 0 ){
					ts->palm_effect_id[i] = 1;
					ts->palm_effect_survive_id[i] = 1;
					SHTPS_LOG_DBG_PRINT("[pen_ignore][%d] servive effect area (%d, %d) - (%d, %d)\n",
											i,
											ts->palm_area_x_min[i], ts->palm_area_y_min[i],
											ts->palm_area_x_max[i], ts->palm_area_y_max[i]);
				}else{
					/* time over */
					ts->palm_effect_id[i] = 0;
					ts->palm_effect_survive_id[i] = 0;
					SHTPS_LOG_DBG_PRINT("[pen_ignore][%d] servive effect disable\n", i);
				}
			}
			else if(ts->fw_report_info.fingers[i].state == SHTPS_TOUCH_STATE_PEN &&
						ts->fw_report_info_store.fingers[i].state == SHTPS_TOUCH_STATE_FINGER)
			{
				ts->palm_effect_id[i] = 1;
				ts->palm_effect_survive_id[i] = 1;
				ts->palm_effect_survive_time[i] = jiffies + msecs_to_jiffies(SHTPS_PEN_IGNORE_SURVIVE_TIME_MS);

				ts->palm_area_x_min[i] = (ts->fw_report_info_store.fingers[i].x < SHTPS_PEN_IGNORE_SURVIVE_AREA) ?
											0 : (ts->fw_report_info_store.fingers[i].x - SHTPS_PEN_IGNORE_SURVIVE_AREA);
				ts->palm_area_x_max[i] = ((ts->fw_report_info_store.fingers[i].x + SHTPS_PEN_IGNORE_SURVIVE_AREA) > (CONFIG_SHTPS_SY3000_LCD_SIZE_X - 1)) ?
											(CONFIG_SHTPS_SY3000_LCD_SIZE_X - 1) : (ts->fw_report_info_store.fingers[i].x + SHTPS_PEN_IGNORE_SURVIVE_AREA);
				ts->palm_area_y_min[i] = (ts->fw_report_info_store.fingers[i].y < SHTPS_PEN_IGNORE_SURVIVE_AREA) ?
											0 : (ts->fw_report_info_store.fingers[i].y - SHTPS_PEN_IGNORE_SURVIVE_AREA);
				ts->palm_area_y_max[i] = ((ts->fw_report_info_store.fingers[i].y + SHTPS_PEN_IGNORE_SURVIVE_AREA) > (CONFIG_SHTPS_SY3000_LCD_SIZE_Y - 1)) ?
											(CONFIG_SHTPS_SY3000_LCD_SIZE_Y - 1) : (ts->fw_report_info_store.fingers[i].y + SHTPS_PEN_IGNORE_SURVIVE_AREA);

				SHTPS_LOG_DBG_PRINT("[pen_ignore][%d] servive effect area enable (%d, %d) - (%d, %d)\n",
										i,
										ts->palm_area_x_min[i], ts->palm_area_y_min[i],
										ts->palm_area_x_max[i], ts->palm_area_y_max[i]);
			}
		...
		for(i = 0; i < fingerMax; i++){
			if(ts->pen_hand_ignore_id[i] == 0){
				/* check palm area touch down */
				for(j = 0; j < fingerMax; j++){
					if(ts->palm_effect_id[j] != 0){
						if( (ts->palm_area_x_min[j] <= info->fingers[i].x) &&
							(info->fingers[i].x <= ts->palm_area_x_max[j]) &&
							(ts->palm_area_y_min[j] <= info->fingers[i].y) &&
							(info->fingers[i].y <= ts->palm_area_y_max[j]) )
						{
							ts->pen_hand_ignore_id[i] = 1;
						}
					}
				}
			}else{
				/* check over palm area */
				is_ignore = 0;
				for(j = 0; j < fingerMax; j++){
					if(ts->palm_effect_id[j] != 0){
						if( (ts->palm_area_x_min[j] <= info->fingers[i].x) &&
			...
			if(ts->pen_hand_ignore_id[i] != 0){
				SHTPS_LOG_DBG_PRINT("[pen_ignore] <%d> is ignored\n", i);
				info->fingers[i].state = SHTPS_TOUCH_STATE_NO_TOUCH;
			}
		}
	}
...
static void shtps_hover_reject_check(struct shtps_rmi_spi *ts, struct shtps_touch_info *info)
{
	...
	if(SHTPS_HOVER_REJECT_ENABLE_SWITCH == 0){
		return;
	}

	for(i = 0; i < fingerMax; i++){
		if(ts->finger_tu_finger_ignore_td_check_enable[i] != 0)
		{
			...
			if(ts->fw_report_info.fingers[i].state != SHTPS_TOUCH_STATE_FINGER){
				ts->finger_tu_finger_ignore_td_cont_check_enable[i] = 0;
			}

			if( ts->fw_report_info.fingers[i].state == SHTPS_TOUCH_STATE_FINGER &&
				ts->fw_report_info_store.fingers[i].state == SHTPS_TOUCH_STATE_NO_TOUCH )
			{
				ts->finger_tu_finger_ignore_td_info.fingers[i].state = SHTPS_TOUCH_STATE_FINGER;
				ts->finger_tu_finger_ignore_td_info.fingers[i].x  = ts->fw_report_info.fingers[i].x;
				ts->finger_tu_finger_ignore_td_info.fingers[i].y  = ts->fw_report_info.fingers[i].y;
				ts->finger_tu_finger_ignore_td_info.fingers[i].wx = ts->fw_report_info.fingers[i].wx;
				ts->finger_tu_finger_ignore_td_info.fingers[i].wy = ts->fw_report_info.fingers[i].wy;
				ts->finger_tu_finger_ignore_td_info.fingers[i].z  = ts->fw_report_info.fingers[i].z;
				ts->finger_tu_finger_ignore_td_time[i] = jiffies;

				if(ts->finger_tu_finger_ignore_count[i] < SHTPS_HOVER_REJECT_FINGER_TU_FINGER_IGNORE_TD_COUNT){
					ts->finger_tu_finger_ignore_count[i]++;
					SHTPS_LOG_DBG_PRINT("[HOVER_REJECT] <%d> finger up finger ignore check td count [%d]\n",
											i, ts->finger_tu_finger_ignore_count[i]);
				}
			}

			if(ts->fw_report_info.fingers[i].state == SHTPS_TOUCH_STATE_FINGER)
			{
				diff_x = shtps_get_diff(ts->finger_tu_finger_ignore_tu_pos[i][SHTPS_POSTYPE_X], ts->fw_report_info.fingers[i].x, SHTPS_POS_SCALE_X(ts));
				diff_y = shtps_get_diff(ts->finger_tu_finger_ignore_tu_pos[i][SHTPS_POSTYPE_Y], ts->fw_report_info.fingers[i].y, SHTPS_POS_SCALE_Y(ts));

				if( time_after(jiffies, ts->finger_tu_finger_ignore_tu_time[i] + msecs_to_jiffies(SHTPS_HOVER_REJECT_FINGER_TU_FINGER_IGNORE_TIME)) == 0 )
				{
					if( (diff_x <= SHTPS_HOVER_REJECT_CANCEL_MOVE_THRESHOLD) &&
						(diff_y <= SHTPS_HOVER_REJECT_CANCEL_MOVE_THRESHOLD) )
					{
						info->fingers[i].state = SHTPS_TOUCH_STATE_NO_TOUCH;
						ts->finger_tu_finger_ignore_td_cont_check_enable[i] = 1;
						SHTPS_LOG_DBG_PRINT("[HOVER_REJECT] <%d> finger ignore by after finger up\n", i);
					}
					else{
						is_check_end = 1;
						SHTPS_LOG_DBG_PRINT("[HOVER_REJECT] <%d> finger up finger ignore check end by move thresh\n", i);
					}
				}
				else{
					is_check_end = 1;
					SHTPS_LOG_DBG_PRINT("[HOVER_REJECT] <%d> finger up finger ignore check end by time over\n", i);
				}

				if(ts->finger_tu_finger_ignore_td_cont_check_enable[i] != 0){
					if( time_after(jiffies, ts->finger_tu_finger_ignore_td_time[i] + msecs_to_jiffies(SHTPS_HOVER_REJECT_FINGER_TU_FINGER_TD_IGNORE_TIME)) == 0 )
					{
						info->fingers[i].state = SHTPS_TOUCH_STATE_NO_TOUCH;
						SHTPS_LOG_DBG_PRINT("[HOVER_REJECT] <%d> finger up finger down ignored\n", i);
					}
					else{
						is_check_end = 1;
						ts->finger_tu_finger_ignore_td_cont_check_enable[i] = 0;
						SHTPS_LOG_DBG_PRINT("[HOVER_REJECT] <%d> finger up finger down ignore check end by time over\n", i);
					}
				}
			}
			else{
			...
			if(ts->finger_tu_finger_ignore_count[i] >= SHTPS_HOVER_REJECT_FINGER_TU_FINGER_IGNORE_TD_COUNT){
				is_check_end = 1;
				SHTPS_LOG_DBG_PRINT("[HOVER_REJECT] <%d> finger up finger ignore check end by td count\n", i);
			}

			if( (is_check_end != 0) && (ts->finger_tu_finger_ignore_td_cont_check_enable[i] == 0) ){
				ts->finger_tu_finger_ignore_td_check_enable[i] = 0;
				SHTPS_LOG_DBG_PRINT("[HOVER_REJECT] <%d> finger up finger ignore check end\n", i);
			}
		}
		...
		if(ts->finger_tu_finger_ignore_td_check_enable[i] == 0)
		{
			if( ts->fw_report_info.fingers[i].state == SHTPS_TOUCH_STATE_NO_TOUCH &&
				ts->fw_report_info_store.fingers[i].state == SHTPS_TOUCH_STATE_FINGER )
			{
				ts->finger_tu_finger_ignore_tu_pos[i][SHTPS_POSTYPE_X] = ts->fw_report_info_store.fingers[i].x;
				ts->finger_tu_finger_ignore_tu_pos[i][SHTPS_POSTYPE_Y] = ts->fw_report_info_store.fingers[i].y;
				ts->finger_tu_finger_ignore_tu_time[i] = jiffies;
				ts->finger_tu_finger_ignore_count[i] = 0;
				ts->finger_tu_finger_ignore_td_check_enable[i] = 1;
			}
		}
			...
			ts->fw_report_info_store.fingers[i].state == SHTPS_TOUCH_STATE_FINGER)
		{
			...
			if(ts->finger_tu_pen_ignore[i] == 0){
					...
					if(ts->tu_check_enable[i] != 0)
					{
							...
							(diff_y <= SHTPS_HOVER_REJECT_FINGER_TU_PEN_IGNORE_MOVE_THRESHOLD) )
						{
							if( time_after(jiffies, ts->tu_time[i] + msecs_to_jiffies(SHTPS_HOVER_REJECT_FINGER_TU_PEN_IGNORE_TIME)) == 0 )
							{
								...
								info->fingers[i].state = SHTPS_TOUCH_STATE_NO_TOUCH;
								SHTPS_LOG_DBG_PRINT("[HOVER_REJECT] <%d> pen ignore by after finger up start\n", i);
							}
						}
					}
				}
				...
				if(info->fingers[i].state == SHTPS_TOUCH_STATE_PEN){
					info->fingers[i].state = SHTPS_TOUCH_STATE_NO_TOUCH;
					SHTPS_LOG_DBG_PRINT("[HOVER_REJECT] <%d> pen ignore by after finger up continue\n", i);
				}else{
					ts->finger_tu_pen_ignore[i] = 0;
					SHTPS_LOG_DBG_PRINT("[HOVER_REJECT] <%d> pen ignore by after finger end\n", i);
				}
			}
		}
	}
...
static void shtps_calc_notify(struct shtps_rmi_spi *ts, u8 *buf, struct shtps_touch_info *info, u8 *event)
{
	...
	shtps_ignore_touch_area_check(ts, info);
...
static int shtps_statef_sleep_enter(struct shtps_rmi_spi *ts, int param)
{
	...
	#if defined(SHTPS_PEN_HAND_FAIL_EVENT_REPORT_REJECTION)
		memset(ts->pen_hand_ignore_id, 0, sizeof(ts->pen_hand_ignore_id));
		memset(ts->palm_effect_id, 0, sizeof(ts->palm_effect_id));
		memset(ts->palm_effect_survive_id, 0, sizeof(ts->palm_effect_survive_id));
	#endif /* SHTPS_PEN_HAND_FAIL_EVENT_REPORT_REJECTION */
...
static int shtps_init_internal_variables(struct shtps_rmi_spi *ts)
{
	...
	memset(ts->drag_hist, 0, sizeof(ts->drag_hist));
		...
		memset(ts->palm_effect_id, 0, sizeof(ts->palm_effect_id));
		memset(ts->palm_effect_survive_id, 0, sizeof(ts->palm_effect_survive_id));
		memset(ts->palm_effect_survive_time, 0, sizeof(ts->palm_effect_survive_time));
		memset(ts->palm_area_x_min, 0, sizeof(ts->palm_area_x_min));
		memset(ts->palm_area_x_max, 0, sizeof(ts->palm_area_x_max));
		memset(ts->palm_area_y_min, 0, sizeof(ts->palm_area_y_min));
		memset(ts->palm_area_y_max, 0, sizeof(ts->palm_area_y_max));
	#endif /* SHTPS_PEN_HAND_FAIL_EVENT_REPORT_REJECTION */
