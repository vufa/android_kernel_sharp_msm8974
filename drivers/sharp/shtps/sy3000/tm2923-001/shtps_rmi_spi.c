 ...
 */
		...
		#include "shtps_fw_tm2923-001_pp1.h"
	#else
		#include "shtps_fw_tm2923-001.h"
...
#define SHTPS_TOUCH_CANCEL_COORDINATES_Y		9999
	...
	#define SHTPS_CLING_REJECT_POS_HIST_MAX		3
		...
		static int SHTPS_CLING_REJECT_MODE0_Z_THRESH =			60;
		static int SHTPS_CLING_REJECT_MODE0_OFFSET_Z =			2;
		...
		static int SHTPS_CLING_REJECT_MODE5_Z_THRESH =			150;
		static int SHTPS_CLING_REJECT_MODE5_OFFSET_Z =			2;
		static int SHTPS_CLING_REJECT_MODE5_CNT =				254;
		static int SHTPS_CLING_REJECT_MODE6_Z_SMALL_THRESH =	90;
		static int SHTPS_CLING_REJECT_MODE6_Z_LARGE_THRESH =	130;
		...
		#define SHTPS_CLING_REJECT_MODE0_Z_THRESH			60
		#define SHTPS_CLING_REJECT_MODE0_OFFSET_Z			2
		...
		#define SHTPS_CLING_REJECT_MODE5_Z_THRESH			150
		#define SHTPS_CLING_REJECT_MODE5_OFFSET_Z			2
		#define SHTPS_CLING_REJECT_MODE5_CNT				254
		#define SHTPS_CLING_REJECT_MODE6_Z_SMALL_THRESH		90
		#define SHTPS_CLING_REJECT_MODE6_Z_LARGE_THRESH		130
		...
		static int SHTPS_DRAG_GHOST_REJECT_Z_THRESHOLD = 		20;
		static int SHTPS_DRAG_GHOST_REJECT_Z_DIFF_THRESHOLD	= 	80;
		...
		#define SHTPS_DRAG_GHOST_REJECT_Z_THRESHOLD				20
		#define SHTPS_DRAG_GHOST_REJECT_Z_DIFF_THRESHOLD		80
...
struct shtps_cling_reject{
	...
	u8									forcecal_after_large_object;
};
#endif /* SHTPS_CLING_REJECTION_ENABLE */
...
static int shtps_cling_reject_no_touch_reaction_check(struct shtps_rmi_spi *ts)
{
	...
	SHTPS_LOG_CLING_REJECT("[mode_4] no touch reaction check <l_obj=%d><f_state[0]=%02X><f_state[1]=%02X><f_state[2]=%02X>\n",
								large_object_preseent, finger_state[0], finger_state[1], finger_state[2])
...
static int shtps_cling_reject_mode0_check(struct shtps_rmi_spi *ts, struct shtps_touch_info *info)
{
	...
	for(i = 0; i < fingerMax; i++){
		if(info->fingers[i].state == SHTPS_TOUCH_STATE_FINGER){
			if(info->fingers[i].x == ts->fw_report_info.fingers[i].x &&
			info->fingers[i].y == ts->fw_report_info.fingers[i].y &&
			info->fingers[i].wx == ts->fw_report_info.fingers[i].wx &&
			info->fingers[i].wy == ts->fw_report_info.fingers[i].wy &&
			info->fingers[i].z < SHTPS_CLING_REJECT_MODE0_Z_THRESH &&
			abs(info->fingers[i].z -ts->fw_report_info.fingers[i].z) <= SHTPS_CLING_REJECT_MODE0_OFFSET_Z)
			{
		...
		if(ts->cling_reject.mode0_cling_cnt[i]>SHTPS_CLING_REJECT_MODE0_CNT){
			cling_detect = 1;
		}
	}
...
static int shtps_cling_reject_mode5_check(struct shtps_rmi_spi *ts, struct shtps_touch_info *info)
{
	...
	if(ts->cling_reject.forcecal_after_large_object == 0){
		return 0;
	}
	...
	if(ts->cling_reject.forcecal_after_large_object == 1){
		if( numOfFingers > 0 ){
			memset(ts->cling_reject.mode5_cling_cnt, 0, sizeof(ts->cling_reject.mode5_cling_cnt));
			ts->cling_reject.forcecal_after_large_object = 2;
			SHTPS_LOG_CLING_REJECT("[mode_5] check start\n");
		}
	}
	
	if(ts->cling_reject.forcecal_after_large_object == 2){
		if(numOfFingers == 0){
			ts->cling_reject.forcecal_after_large_object = 0;
...
static void shtps_wakeup_calib_delayed_work_function(struct work_struct *work)
{
	...
	if(ts->wakeup_calib_enable != 0)
	{
		...
		if( (numOfReportFingers == 0) && (key_state == 0) ){
			...
			#if defined(SHTPS_CLING_REJECTION_ENABLE)
				if(large_object_preseent != 0){
					ts->cling_reject.forcecal_after_large_object = 1;
				}
			#endif /* SHTPS_CLING_REJECTION_ENABLE */
